-- =====================================================
-- 🚀 MOCARDS CLOUD DEPLOYMENT SCHEMA FOR SUPABASE (FIXED)
-- Complete setup for production deployment - FIXED VERSION
-- Run this in your Supabase SQL Editor for full functionality
-- Handles all dependency issues and column creation order
-- =====================================================
...
-- =====================================================
-- 2. CARD BATCHES TABLE (Create before cards table)
-- =====================================================
...
-- =====================================================
-- 3. ENHANCED CARDS TABLE WITH ALL COLUMNS
-- =====================================================
...
CREATE TABLE IF NOT EXISTS cards (
    ...
    -- NEW: MOC Control number (PRIMARY IDENTIFIER)
...
-- =====================================================
-- 5. PERFORMANCE INDEXES (After all tables created)
-- =====================================================
...
-- Card indexes (NOW SAFE TO CREATE)
...
-- =====================================================
-- 6. DATABASE FUNCTIONS FOR MOC CARD GENERATION
-- =====================================================

-- Generate unique control number with exact format - SAFE VERSION
CREATE OR REPLACE FUNCTION generate_control_number_safe(
...
BEGIN
    -- Check if cards table exists and has control_number column
    SELECT EXISTS (
    ...
    LOOP
        ...
        -- Check uniqueness only if table exists
        IF NOT v_table_exists OR NOT EXISTS (SELECT 1 FROM cards WHERE control_number = v_control_number) THEN
...
-- Create the main function that uses the safe version
CREATE OR REPLACE FUNCTION generate_control_number(
    p_prefix VARCHAR(5) DEFAULT 'MOC',
    p_sequence INTEGER DEFAULT 1
) RETURNS VARCHAR(50) AS $$
BEGIN
    RETURN generate_control_number_safe(p_prefix, p_sequence);
END;
$$ LANGUAGE plpgsql;
...
-- Card history tracking (SAFE VERSION)
...
-- =====================================================
-- 9. ADMIN SETUP AND SAMPLE DATA
-- =====================================================

-- Create instant admin (modify email as needed)
...
-- =====================================================
-- 10. VERIFICATION AND STATUS CHECK
-- =====================================================
...
-- Test control number generation (SAFE VERSION)
SELECT
...
SELECT
    ...
    CASE
        ...
        THEN '✅ Admin ready! Use Supabase Auth to create admin user with email: admin@mocards.com'
